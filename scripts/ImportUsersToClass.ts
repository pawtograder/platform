...
async function main() {
  ...
  const csv = createReadStream("3500-fix-enrollments.csv").pipe(
    parse({
      columns: true,
  ...
  const { data: course } = await supabase.from("classes").select("*, class_sections(*)").eq("id", COURSE_ID).single();
  for await (const record of csv) {
    const user = allUsers.data!.users.find((dbUser) => record.Email === dbUser.email);
    const classSection = course!.class_sections.find(
      (section) => section.canvas_course_section_id === parseInt(record["Canvas Section ID"])
    );
