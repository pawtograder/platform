...
if compgen -G "coverage/server-cdp*.json" >/dev/null || compgen -G "coverage/build-cdp*.json" >/dev/null; then
  ...
  # Give the converter a large heap. Even after merging the ~130 build dumps +
  # runtime dump down to ~240 distinct scripts (see v8-server-to-lcov.ts) and
  # de-duplicating source/map reads, monocart still holds the unminified
  # sources + sourcemaps + coverage model in memory. With the node default —
  # and even at 8 GB — this OOMs ("Reached heap limit"), producing NO
  # server.lcov, so the next-server flag silently uploads 0 files. The
  # converter runs alone here (build finished, Next stopped), so it can use
  # most of the runner; the build already sustains --max-old-space-size=8000
  # *plus* webpack workers, so 12 GB for a single process is within budget.
  NODE_OPTIONS="--max-old-space-size=12288" npx tsx scripts/coverage/v8-server-to-lcov.ts \
