      ...
      if (isCompleted) {
        ...
        for (const crit of criteria) {
          for (const { check, points, label } of selectAppliedChecks(crit)) {
            ...
            const targetProfile = needsIndividualTarget
              ? defaultTargetBySubmission.get(review.submission_id)
              : undefined;
...
public class Entrypoint {
  ...
  public String getMessage() {
      ...
      return "Hello, World!";
        ...
        // Precompute clustered autograder test results per submission so the parent
        // grader_results score matches the child tests and failures cluster by name.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const testResultsPerSubmission = submissionData.map((submission: any) => computeAutograderTests(submission.id));

        // Prepare grader results for this chunk
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const graderResultInserts = submissionData.map((submission: any, index) => {
          const tests = testResultsPerSubmission[index];
        ...
        // Prepare grader result tests for this chunk from the precomputed,
        // clustered results (functional tests + one AI-hint test per submission).
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const graderResultTestInserts = graderResultData.flatMap((graderResult: any, index) =>
          testResultsPerSubmission[index].map((t) => ({
            score: t.score,
            ...
            // Test Insights RPCs (get_test_statistics_for_assignment / _common_test_errors_
            // for_assignment / _submissions_to_full_marks) INNER JOIN submissions via this
            // column; without it the dashboard panels are empty.
            submission_id: (submissionData[index] as { id: number }).id,
            is_released: true,
