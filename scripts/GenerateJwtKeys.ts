...
//
// Outputs (in --env mode; --helm-values / --kv produce the same content
...
//   REALTIME_ENC_KEY             — 16-byte AES key for tenant secret encryption
//   PG_META_CRYPTO_KEY           — 32-byte AES key for pg-meta saved connections
//   PGSODIUM_ROOT_KEY            — 32-byte hex key for pgsodium
...
//   SUPAVISOR_METRICS_JWT_SECRET — signs /metrics scrape JWTs
//
// Usage:
...
// Mirrors the generation in charts/.../secrets-bootstrap-script.yaml so
// the two paths (in-cluster autogenerate vs. operator-run script) stay
// in lockstep.
const supavisorSecretKeyBase = randomBytes(48).toString("base64"); // ~64 chars (Phoenix endpoint key)
const supavisorVaultEncKey = randomBytes(24).toString("base64").slice(0, 32); // exactly 32 bytes
const supavisorApiJwtSecret = randomBytes(48).toString("base64");
const supavisorMetricsJwtSecret = randomBytes(48).toString("base64");
const octPublic = {
  kty: "oct",
  k: b64url(hsSecret),
  kid: APIKEYS_KID,
...
if (mode === "env") {
  ...
  // Mirrors every field required by templates/secrets.yaml when
  // secrets.create=true. Keep in sync with that template.
