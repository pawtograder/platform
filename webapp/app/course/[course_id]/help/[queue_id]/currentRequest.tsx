...
function ChatChannelParticipants() {
    ...
    return <>
        {participants.map((participant, index) => (
            <Text key={participant}>{profiles.users.find((p) => p.id === participant)?.name}{index === participants.length - 1 ? "" : ", "}</Text>
        ))}
    </>
}
function HelpRequestStudentActions({ request }: { request: HelpRequest }) {
    if (request.is_video_live)
        return <>
            ...
            variant="ghost"
            onClick={() => {
              window.open(`${process.env.NEXT_PUBLIC_PAWTOGRADER_WEB_URL}/course/${request.class_id}/help/${request.help_queue}/request/${request.id}/meet`, '_blank');
            }}
    ...
    return <Box width="100%">
        <DataList.Root>
            ...
            <DataList.Item>
                ...
                <DataList.ItemValue>
                    {assignee ? assignee.name : "Nobody is working on this request yet"}
                </DataList.ItemValue>
            </DataList.Item>
        </DataList.Root>
