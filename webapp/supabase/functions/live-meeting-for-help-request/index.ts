...
async function handleRequest(req: Request) {
  const { courseId, helpRequestId } = await req
    .json() as LiveMeetingForHelpRequestRequest;
  const supabase = await assertUserIsInCourse(
    courseId,
  ...
  const { data: helpRequest } = await adminSupabase.from("help_requests")
    .select("*").eq("id", helpRequestId).single();
  if (!helpRequest) {
  ...
  try {
    ...
    Meeting = await chime.getMeeting({
      MeetingId: (await chimeUtils.getActiveMeeting(helpRequest))
        .chime_meeting_id!,
    });
  }
  if (!Meeting) {
