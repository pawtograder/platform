...
export async function processSNSMessage(message: ChimeSNSMessage): Promise<void> {
    ...
    if (message.detail.eventType === "chime:AttendeeLeft") {
        ...
        await adminSupabase.from("video_meeting_sessions").update({
            ended: new Date().toISOString(),
        }).eq("id", sessionID);
        await adminSupabase.from("help_requests").update({
            is_video_live: false,
        }).eq("id", helpRequestID);
...
export async function getActiveMeeting(
    ...
    const activeMeeting = await adminSupabase.from("video_meeting_sessions")
        .select("*").eq(
            "help_request_id",
            helpRequest.id,
        )
    ...
    if (!activeMeeting.data) {
        ...
        const { error: updateError } = await adminSupabase.from(
