    ...
    <Navbar className="nav" flexDirection="column" container>
      ...
      <Flex css="margin-top: 0rem;">
        ...
        {layout === Layout.Gallery && priorityBasedPolicy && (
          <>
            <NavbarItem icon={<ZoomIn />} onClick={zoomIn} label="Zoom In" disabled={!!sharingAttendeeId} />
            <NavbarItem icon={<ZoomOut />} onClick={zoomOut} label="Zoom Out" />
          </>
        )}
      </Flex>
      <Flex marginTop="auto">
        <NavbarItem icon={<Eye />} onClick={toggleTheme} label={theme === "light" ? "Dark mode" : "Light mode"} />
        <NavbarItem
          icon={<SignalStrength />}
          onClick={(): void => {
            // do nothing
