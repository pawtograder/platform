...
The optional Discord bot integration posts help-request and regrade notifications
to Discord channels and links staff Discord accounts for @-mentions. Setup
(application, bot token, GoTrue `discord` OAuth provider, webhook keys, and the
`discord-async-worker` Edge Function) is documented in
**[DEPLOYMENT.md → Discord notifications](./DEPLOYMENT.md#discord-notifications)**.
