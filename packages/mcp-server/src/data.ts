/**
 * Data access layer for Pawtograder MCP Server
...
async function getSafeProfile(
  ...
  // If we need to check privacy, we'd do it in a separate query
  return {
    id: data.id,
    name: data.name,
 ...
 * Returns "Anonymous" for private profiles or if profile not found
 */
...
export async function getAssignment(
  ...
  const { data, error } = await supabase
    .from("assignments")
    .select(
      "id, title, slug, description, handout_url, due_date, release_date, total_points, has_autograder, class_id"
    )
    .eq("id", assignmentId)
    .eq("class_id", classId)
    .single();
...
export async function getHelpRequest(
  ...
  // Get help queue name
  const helpQueueName =
    (helpRequest.help_queues as unknown as { name: string })?.name || "Unknown Queue";
...
export async function getSubmission(
  ...
  const { data: submission, error: subError } = await supabase
    .from("submissions")
    .select(
      "id, assignment_id, created_at, sha, repository, ordinal, is_active, profile_id"
    )
    .eq("id", submissionId)
    .eq("class_id", classId)
    .single();
  ...
  const { data: graderData } = await supabase
    .from("grader_results")
    .select(
      "id, score, max_score, lint_passed, lint_output, lint_output_format, errors, execution_time, ret_code"
    )
    .eq("submission_id", submissionId)
    .eq("class_id", classId)
    .order("created_at", { ascending: false })
  ...
  if (graderData) {
    ...
    const { data: testsData } = await supabase
      ...
      .select(
        "id, name, part, score, max_score, output, output_format, is_released"
      )
...
export async function searchDiscussionThreads(
  ...
  if (options.searchQuery) {
    query = query.or(
      `subject.ilike.%${options.searchQuery}%,body.ilike.%${options.searchQuery}%`
    );
  }
