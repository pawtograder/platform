...
| 1.3 | Backup is a plain-SQL `pg_dump \| gzip` with **no integrity verification** before or after upload | `backup.yaml` | ✅ switched to `pg_dump -Fc` (custom format, internally compressed), TOC verified with `pg_restore --list`, post-upload size check |
...
| 2.2 | No `securityContext` on any workload except smtp-relay (no runAsNonRoot, no capability drop, no seccomp) | all | ✅ values-driven pod/container securityContext helpers, conservative defaults (seccomp `RuntimeDefault`, `allowPrivilegeEscalation: false`, drop ALL); postgres and realtime exempt from drop-ALL (postgres entrypoint needs setuid; realtime's entrypoint `sudo`s to `nobody` and needs the default capability set, `containerSecurityContext: {}`) |
...
| 2.8 | `web.workflowMetricsLeader` is an honor-system singleton flag | `web.yaml` | ✅ guarded: chart fails when leader=true with replicas>1. Chart 0.3.18 added the missing other half — that guard left a multi-replica web tier with **no** way to export the workflow gauges, so the nine workflow panels on `app-business` were permanently empty in production. `web.metricsLeader.enabled` renders a dedicated 1-replica Deployment (`templates/web-metrics-leader.yaml`, component `metrics-leader`, `strategy: Recreate`, `replicas` deliberately not a value) with its own Service + ServiceMonitor and off every user-traffic path. Enabling both mechanisms is refused, as is a multi-replica install with monitoring on and neither — the latter cleared by `monitoring.allowMissingWorkflowMetrics: true`. Refresh frequency is additionally bounded app-side by `METRICS_WORKFLOW_REFRESH_INTERVAL_SECONDS` (default 300), which is the only bound that survives a second Prometheus. |
...
| 3.2 | One shared ServiceAccount, token auto-mounted into every pod | `serviceaccount.yaml` | ✅ `automountServiceAccountToken: false` on the shared SA, which every pod (including smtp-relay) now uses; only the bootstrap job opts back in |
