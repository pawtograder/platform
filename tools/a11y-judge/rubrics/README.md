...
SC 2.4.11 Focus Appearance is referenced in the 2.4.7 rubric as _informative_
...
| ------------------------------- | ----- | ----- | -------------------------------------------------------------- |
| `1.1.1-non-text-content.md`     | 1.1.1 | A     | alt-text _quality/adequacy_ in context (presence is axe's job) |
...
- **`verdict`** — `pass` | `fail` | `needs_human`.
  ...
  rubric's _needs_human triggers_). It is not a soft fail.
...
- **`severity` 1–5** — 5 = blocks task completion for the affected user group;
  1 = cosmetic. See each rubric's _severity guidance_ for the per-criterion mapping.
...
a rubric changes only _that criterion's_ `rubricHash`, so it invalidates only that
