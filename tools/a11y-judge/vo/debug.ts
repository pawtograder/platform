/**
 * Always-on diagnostic log for the real-VoiceOver runner. Every entry is
 * mirrored to the console (CI log) AND appended immediately to
 * a11y-vo-artifacts/<runId>/debug/debug.jsonl, so a run that dies mid-login
 * still leaves a complete trace in the uploaded artifact.
 */
...
export class VoDebugLog {
  ...
  log(stage: string, detail?: Record<string, unknown>): void {
    ...
    console.log(`[a11y:vo:debug] ${stage}${detail ? " " + JSON.stringify(detail) : ""}`);
    this.append(entry);
  }
  ...
  step(record: AtStepRecord): void {
    ...
    console.log(
      `[a11y:vo:debug] cmd#${record.index} ${record.command}${record.arg ? `(${JSON.stringify(record.arg)})` : ""} ` +
  ...
  /**
   * Best-effort full-screen screenshot into the debug dir. Needs the Screen
   * Recording TCC grant (same one @guidepup/record uses); without it macOS
   * still writes an image, just without window contents.
   */
  async screenshot(label: string): Promise<void> {
    const file = path.join(this.dir, `${label.replace(/[^a-zA-Z0-9_-]+/g, "-")}-${Date.now()}.png`);
    await new Promise<void>((resolve) => {
      execFile("screencapture", ["-x", file], { timeout: 10_000 }, (error) => {
