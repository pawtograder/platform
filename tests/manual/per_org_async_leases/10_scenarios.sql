...
begin
  ...
  for i in 1..20 loop
    select count(*) into v_rows
      from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's1-h' || i, 300, 3, 16);
    if v_rows > 0 then
...
begin
  ...
  for i in 1..30 loop
    select count(*) into v_rows
      from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's4-h' || i, 300, 10, 5);
...
begin
  ...
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's6-owner', 2, 4, 1);
  perform harness.expect('6 lease lifecycle', 'owner claims', '4', v_rows::text);

  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's6-rival', 2, 4, 1);
  ...
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's6-lapser', 1, 4, 1);
  perform harness.expect('6 lease lifecycle', 'claim after release', '4', v_rows::text);

  perform pg_sleep(1.5);
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's6-reclaimer', 60, 4, 1);
...
begin
  ...
  -- target((unresolved)) = least(ceil(8/4), 4) = 2 at the start -- it has to compete for slots with
  -- a 40-message real org and win at least one.
  for i in 1..6 loop
  ...
  perform harness.expect('7 sentinel org', 'unresolved messages delivered in phase A', '4',
    (select count(*)::text from harness.claims
      where scenario = '7 sentinel org' and org = '(unresolved)'));
  -- Only one slot, not two, and that is correct rather than a shortfall: after the first read the
  -- sentinel has 4 ready messages left, so target drops to least(ceil(4/4), 4) = 1 and the one slot
  -- it already holds IS its allocation. Demand is measured on READY messages, so work already
  -- handed to a leaseholder correctly stops counting as unmet demand.
  perform harness.expect('7 sentinel org', 'slots held by the sentinel after phase A', '1',
    (select count(*)::text from public.async_worker_slots
      where expires_at > clock_timestamp() and org = '(unresolved)'));
...
begin
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's9-h1', 300, 8, 16);
  ...
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 's9-h2', 300, 8, 16);
...
declare
  ...
  v_main_wins int := 0;
begin
  -- Five holders hammer orglease_alt with global_cap = 2.
  for i in 1..5 loop
    insert into harness.claims(scenario, holder, org, msg_id)
  ...
  -- Now the other queue, with a DIFFERENT cap. Five live slots overall -- more than either cap --
  -- is the proof the two budgets are independent rather than shared.
  for i in 1..5 loop
    insert into harness.claims(scenario, holder, org, msg_id)
    select '10 two queues main', 'main-h' || i, t.org, t.msg_id
      from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 'main-h' || i, 300, 4, 3) t;
    get diagnostics v_rows = row_count;
    if v_rows > 0 then v_main_wins := v_main_wins + 1; end if;
  end loop;

  perform harness.expect('10 two queues', 'main claims that won a slot (cap 3, of 5)', '3', v_main_wins::text);
  ...
  perform harness.expect('10 two queues', 'alt pool untouched by the async_calls claims', '2',
    (select count(*)::text from public.async_worker_slots
      where queue_name = 'orglease_alt' and expires_at > clock_timestamp()));
  perform harness.expect('10 two queues', 'live slots overall exceed either cap', '5',
    (select count(*)::text from public.async_worker_slots where expires_at > clock_timestamp()));
...
begin
  ...
  -- would silently cap the configured budget, which is the same silence in a subtler dress.
...
-- expired: a phantom leaseholder consuming global_cap and pinning its org below target for as long
-- as the worker lived. Queue scoping bounds that at one TTL -- the same bound a crashed isolate
-- already has.
select harness.reset('async_calls');
select pgmq.purge_queue('async_calls_low_priority');
...
begin
  ...
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 'rotator', 2, 2, 8);
  perform harness.expect('12 queue-scoped lease', 'claim on async_calls', '4', v_rows::text);
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls_low_priority', 300, 4, 'rotator', 2, 2, 8);
  ...
  perform harness.expect('12 queue-scoped lease', 'low-priority lease survives', '1',
    (select count(*)::text from public.async_worker_slots
      where queue_name = 'async_calls_low_priority' and expires_at > clock_timestamp()));
  -- The freed slot is usable by someone else, i.e. it is genuinely not a phantom any more.
  select count(*) into v_rows
    from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 'rival', 60, 2, 8);
...
begin
  ...
  for i in 1..20 loop
    select count(*) into v_rows
      from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 'cost250-h' || i, 300, 8, 32);
  end loop;
...
begin
  ...
  for i in 1..20 loop
    select count(*) into v_rows
      from pgmq_public.claim_org_slot_and_read('async_calls', 300, 4, 'cost-h' || i, 300, 8, 32);
  end loop;
  v_claim_ms := extract(epoch from (clock_timestamp() - v_t0)) * 1000 / 20;
