...
async function clickWithTextboxRetry(page: Page, clickTarget: ReturnType<Page['getByLabel']>, textboxSelector: ReturnType<Page['getByRole']>, maxRetries = 3) {
...
test.beforeAll(async () => {
  ...
  const { data: repositoryData, error: repositoryError } = await supabase.from("repositories").insert({
    assignment_id: 1,
  ...
  const { data: checkRunData, error: checkRunError } = await supabase.from("repository_check_runs").insert({
    class_id: 1,
  ...
  const { data: submissionData, error: submissionError } = await supabase.from("submissions").insert({
    assignment_id: 1,
    profile_id: student_private_profile_id.data?.private_profile_id,
    sha: "none",
...
public class Entrypoint {
  ...
  const { data: graderResultData, error: graderResultError } = await supabase.from("grader_results").insert({
    submission_id: submission_id,
    score: 5,
    class_id: 1,
...
test.describe('An end-to-end grading workflow self-review to grading', () => {
  ...
  test('Instructors can view the student\'s self-review and create their own grading review', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Add Check' }).waitFor({ state: 'visible', timeout: 1000 });
    await page.getByRole('textbox', { name: 'Optional: comment on check Grading Review Check 2' }).fill('grading comment');
    await page.getByRole('button', { name: 'Add Check' }).click();
    // await clickAddCheckWithRetry(page);
