...
/** Email + password sign-in (stable in local dev; magic links can abort navigation in some environments). */
export async function loginAsUserWithPassword(
  page: Page,
  testingUser: TestingUser,
  course?: Course,
  dismissTimezoneDialog = true
) {
  if (dismissTimezoneDialog) {
    await ensureTimeZonePreferenceInitialized(page);
  }
...
export async function createUsersInClass(
  ...
  for (const request of resolvedRequests) {
    ...
    const e2ePassword = process.env.TEST_PASSWORD || "change-it";
    const { error: passwordUpdateError } = await supabase.auth.admin.updateUserById(userId, { password: e2ePassword });
    if (passwordUpdateError) {
      throw new Error(`Failed to set password for ${resolvedEmail}: ${passwordUpdateError.message}`);
    }
