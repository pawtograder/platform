...
import { createClass, createClassSection, createLabSectionWithStudents, createUserInClass, createUserInDemoClass, loginAsUser, supabase, TestingUser } from "./TestingUtils";
...
async function sendBatchEmails({ page, target_text, class_section_text, lab_section_text }: { page: Page, target_text: string, class_section_text?: string, lab_section_text?: string }) {
...
async function expectStudentsReceivedExactlyTheseEmails({ students, expected_emails }: {
    ...
    const { data: allEmailsSent, error: emailError } = await supabase.from("emails").select("*").eq("class_id", course.id).limit(1000);
    if (emailError) {
        throw new Error(`Error getting emails: ${emailError.message}`);
    }
    const { data: allNotifications, error: notificationError } = await supabase.from("notifications").select("*").eq("class_id", course.id).limit(1000);
    if (notificationError) {
        throw new Error(`Error getting notifications: ${notificationError.message}`);
    }
    async function expectStudentReceivedEmail({ student, expected_email }: {
        student: TestingUser,
        expected_email: {
            subject: string,
            body: string
        ...
        const thisEmail = allEmailsSent?.find(email => email.subject === expected_email.subject && email.body === expected_email.body);
        expect(thisEmail, `Expected email ${expected_email.subject} for student ${student.email}, but got ${thisEmail}`).toBeDefined();
        const thisNotification = allNotifications?.find(notification => notification.user_id === student.user_id && notification.subject === expected_email.subject);
        expect(thisNotification, `Expected notification ${expected_email.subject} for student ${student.email}, but got ${thisNotification}`).toBeDefined();
        if (thisNotification) {
    ...
    for (const student of students) {
        const totalNotifications = allNotifications?.filter(notification => notification.user_id === student.user_id && notification.style === 'email').length;
        expect(totalNotifications, `Expected ${expected_emails.length} emails for student ${student.email}, but got ${totalNotifications}`).toEqual(expected_emails.length);
        const totalEmails = allEmailsSent?.filter(email => email.user_id === student.user_id).length;
        expect(totalEmails, `Expected ${expected_emails.length} emails for student ${student.email}, but got ${totalEmails}`).toEqual(expected_emails.length);
