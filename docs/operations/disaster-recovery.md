...
1. **Stop writers.** Scale the write tiers to zero so PostgREST/edge/realtime
   ...
   (
     ...
     #    edge-functions HPA's YAML. Step 3 (“Bring writers back”) said "scaling the tiers to their prod replica counts" and had nothing to
     ...
     #    "restored".
     STATE_DIR="/tmp/dr-fence-$(date +%s)"; mkdir -p "$STATE_DIR"
     ...
     echo "prior state recorded in $STATE_DIR -- step 3 reads it back. Note the path down."
...
3. **Bring writers back — reverse step 1, from what step 1 recorded.** Resume
   ...
   STATE_DIR="$(ls -dt /tmp/dr-fence-* | head -1)"
