...
export function useDiscussionThreadWatchStatus(threadId: number) {
    ...
    useEffect(() => {   
        const { unsubscribe, data } = controller.getValueWithSubscription<DiscussionThreadWatcher>("discussion_thread_watchers", threadId, (data) => {
            setCurWatch(data);
        });
        setCurWatch(data);
        return unsubscribe;
    ...
    const setThreadWatchStatus = useCallback(async (status: boolean) => {
        if (curWatch) {
            await updateWatch({
                id: curWatch.id,
                values: {
