...
import { useIsOrphanedRegradeRequest, useRegradeRequestCheckName } from "@/hooks/useOrphanedRegradeRequest";
...
function OrphanedRegradeRequestAlert({ request }: { request: RegradeRequest }) {
  if (request.resolution_reason === "comment_deleted") {
    return (
      <Alert status="warning" title="Action needed">
        The grade this request referenced was changed during re-grading — review the updated grade and escalate if you
        still disagree.
      </Alert>
    );
  }
  ...
  if (request.status === "escalated") {
    return (
      <Alert status="info" title="Grade updated">
        The grade this request referenced was removed during re-grading. Your escalated request and discussion history
        are preserved below for the instructor.
      </Alert>
    );
  }

  return (
    <Alert status="warning" title="Grade updated">
      The grade this request referenced was removed during re-grading. Review the discussion below and escalate if you
      still disagree.
    </Alert>
  );
}

function OrphanedRegradeRequestPanel({ request }: { request: RegradeRequest }) {
  const checkName = useRegradeRequestCheckName(request);
  const rubricCheck = useRubricCheck(request.rubric_check_id);

  return (
    <Box id={`regrade-request-${request.id}`} w="100%">
      <RegradeRequestWrapper regradeRequestId={request.id}>
        <Box borderWidth="1px" borderColor="border.warning" borderRadius="md" p={2} w="100%" fontSize="sm">
          <Text fontSize="sm" fontWeight="semibold" mb={1}>
            {checkName}
          </Text>
          {rubricCheck?.description && (
            <Text fontSize="xs" color="fg.muted" mb={2}>
              {rubricCheck.description}
            </Text>
          )}
          <OrphanedRegradeRequestAlert request={request} />
        </Box>
...
/**
 * Compact summary row for regrade requests that still have a live anchor in the rubric sidebar.
 */
function AnchoredRegradeRequestRow({ request }: { request: RegradeRequest }) {
  const rubricCheck = useRubricCheck(request.rubric_check_id);
  const checkName = useRegradeRequestCheckName(request);
  ...
  return (
    <Box borderWidth="1px" borderColor="border.subtle" borderRadius="md" p={2} w="100%">
...
function RegradeRequestRow({ request }: { request: RegradeRequest }) {
  const isOrphaned = useIsOrphanedRegradeRequest(request);

  if (isOrphaned) {
    return <OrphanedRegradeRequestPanel request={request} />;
  }

  return <AnchoredRegradeRequestRow request={request} />;
}
 ...
 * (draft/opened/escalated) are shown first, followed by resolved/closed ones. Requests whose
 * backing comment was soft-deleted render their full thread here via RegradeRequestWrapper.
 *
