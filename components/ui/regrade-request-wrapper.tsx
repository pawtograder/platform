...
import { useClassProfiles, useIsGraderOrInstructor, useIsInstructor, useRoleByPrivateProfileId } from "@/hooks/useClassProfiles";
                    ...
                    >
                        ...
                        <HStack>
                            {authorRole?.role === "grader" || authorRole?.role === "instructor" || authorProfile?.flair ? (
                                <Tag.Root size="md" colorPalette={authorRole?.role === "grader" || authorRole?.role === "instructor" ? "blue" : authorProfile?.flair_color} variant="surface">
                                    <Tag.Label>{authorRole?.role === "grader" ? "Grader" : authorRole?.role === "instructor" ? "Instructor" : authorProfile?.flair}</Tag.Label>
                                </Tag.Root>
                            ) : (
                                <></>
                            )}
...
export function RegradeRequestComments({ regradeRequestId }: { regradeRequestId: number }) {
    ...
    return <VStack px={2}>{comments?.map((comment) => <RegradeRequestComment key={comment.id} comment={comment} />)}</VStack>;
}
            ...
            >
                ...
                >
                    ...
                    <Box
                        ...
                        borderColor={config.borderColor}
                    >
                        <HStack alignItems="flex-start"><Icon as={StatusIcon} boxSize={3} />
                            <VStack align="flex-start" gap={0} flexGrow={10}><Heading size="sm">Regrade {config.label}</Heading>
                                {regradeRequest.opened_at && <Text fontSize="xs" color="fg.muted">Opened {formatRelative(regradeRequest.opened_at, new Date())}, initial score: {regradeRequest.initial_points || 0}</Text>}
                                {regradeRequest.assignee && <Text fontSize="xs" color="fg.muted">Assigned to {assignee?.name}</Text>}
                                {regradeRequest.resolved_at && <Text fontSize="xs" color="fg.muted">Resolved {formatRelative(regradeRequest.resolved_at, new Date())} by {resolver?.name}, new score: {regradeRequest.resolved_points || 0}</Text>}
                                {regradeRequest.escalated_at && <Text fontSize="xs" color="fg.muted">Appealed {formatRelative(regradeRequest.escalated_at, new Date())} by {escalator?.name}</Text>}
                                {regradeRequest.closed_at && <Text fontSize="xs" color="fg.muted">Closed {formatRelative(regradeRequest.closed_at, new Date())} by {closer?.name}, final score: {regradeRequest.closed_points || 0}</Text>}
                            </VStack>
                                    ...
                                    <PopoverContent>
                                        ...
                                        <PopoverBody>
                                            <VStack gap={3} align="start">
                                                ...
                                                <Text fontSize="sm">
                                                    Enter the final score for this comment after reviewing the regrade request. It will overwrite the score for the comment.
                                                </Text>
                                                <VStack gap={2} align="start" w="100%">
                                                    <Text fontSize="sm" fontWeight="medium">Final Points:</Text>
                                                    <Input
                                                        type="number"
                                    ...
                                    <DialogContent>
                                        ...
                                        <DialogBody>
                                            <VStack gap={4} align="start">
                                                <Text>
                                                    You can appeal this regrade request to an instructor for final review.
                                                </Text>
                                                <Box bg="bg.info" p={3} borderRadius="md" w="100%">
                                                    ...
                                                    <VStack gap={1} align="start">
                                                        ...
                                                        <Text fontSize="sm">
                                                            • Don&apos;t appeal simply because you disagree with the grade
                                                        </Text>
                                                        <Text fontSize="sm">
                                                            • The instructor&apos;s decision will be final
                                                        </Text>
                                                        <Text fontSize="sm">
                                                            • Frivolous appeals may affect future regrade requests
                                                        </Text>
                                                    </VStack>
                                                </Box>
                                            </VStack>
                                    ...
                                    <PopoverContent>
                                        ...
                                        <PopoverBody>
                                            <VStack gap={3} align="start">
                                                ...
                                                <Text fontSize="sm">
                                                    Enter the final decision for this appealed regrade request.
                                                </Text>
                                                <Text fontSize="sm">
                                                    Initial score: {regradeRequest.initial_points}.
                                                </Text>
                                                <Text fontSize="sm">
                                                    Revised score: {regradeRequest.resolved_points}.
                                                </Text>
                                                <VStack gap={2} align="start" w="100%">
                                                    <Text fontSize="sm" fontWeight="medium">Final Points:</Text>
                                                    <Input
                        ...
                        >
                            <VStack p={2} gap={0} align="start">
                                ...
                                <Text fontSize="xs" color="fg.muted">
                                    This comment will be visible to {isGroupAssignment ? "your groupmates and the entire course staff." : "the entire course staff and the author of the submssion."}
                                </Text>
                            </VStack>
                        </Box>
                    )}
