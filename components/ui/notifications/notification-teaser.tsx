...
function AssignmentGroupMemberNotificationTeaser({ notification }: { notification: Notification }) {
    ...
    return <HStack align="flex-start" color="text.muted">
        <Text>{body.name} {body.action === "join" ? "joined" : "left"} your group {body.assignment_group_name} for {body.assignment_name}{body.action === "join" ? ` (added by ${body.added_by_name})` : ""}</Text>
    </HStack>
}
...
function AssignmentGroupJoinRequestNotificationTeaser({ notification }: { notification: Notification }) {
    ...
    if (body.status === "pending") {
        message = <Text>{body.requestor_name} requested to join {body.assignment_group_name} for {body.assignment_name}</Text>
    } else if (body.status === "approved") {
        message = <Text>{body.decision_maker_name} approved {body.requestor_name}'s request to join {body.assignment_group_name} for {body.assignment_name}</Text>
    } else if (body.status === "rejected") {
        message = <Text>{body.decision_maker_name} rejected {body.requestor_name}'s request to join {body.assignment_group_name} for {body.assignment_name}</Text>
