...
function LateTokenButton({ assignment }: { assignment: Assignment }) {
    ...
    const lateTokensAppliedToAssignment = lateTokens.filter(e => e.assignment_id === assignment.id).map(e => e.tokens_consumed).reduce((a, b) => a + b, 0);
    ...
    const extensionsInHours = lateTokens.filter(e => e.assignment_id === assignment.id).map(e => e.hours).reduce((a, b) => a + b, 0);
    ...
    return <Dialog.Root open={open} onOpenChange={(details) => {
        setOpen(details.open);
    }}>
        <Toaster />
        <Dialog.Trigger asChild>
            <Button size="xs" variant="surface" colorPalette="yellow">Extend Due Date</Button>
        </Dialog.Trigger>
        <Dialog.Backdrop />
        <Dialog.Positioner>
            <Dialog.Content>
                <Dialog.Header>
                    ...
                    <Dialog.Description>The course late policy grants each student {course.late_tokens_per_student} late tokens.
                        Each token extends the due date by 24 hours, but are not automatically applied - to use them, you must use this form to apply them BEFORE the assignment is due. You can apply up to {assignment.max_late_tokens} tokens to this assignment.
                        You have already applied {lateTokensAppliedToAssignment} tokens to this assignment.
                        {assignment.max_late_tokens > 1 && <>Note that to apply multiple tokens, you must use this form multiple times, always being sure to extend the due date before the previous due date passes.</>}
                    </Dialog.Description>
                </Dialog.Header>
                <Dialog.Body>
                    ...
                    <Text>You can extend the due date for this assignment by up to {assignment.max_late_tokens - lateTokensAppliedToAssignment} more tokens. Each token extends the due date by 24 hours.</Text>
                    <Alert status="warning" mt={2}>
                        <Text>Once you consume a late token, it is consumed immediately. You will not be able to undo this action.</Text>
                    </Alert>
                        ...
                        onClick={async () => {
                            try {
                                ...
                                toaster.create({
                                    ...
                                    description: "An error occurred while consuming the late token. Please try again, and reach out to your instructor if the problem persists.",
                                    type: "error",
                                })
                            }
                        }}>Consume a late token for a 24 hour extension</Button>
                </Dialog.Body>
            </Dialog.Content>
        </Dialog.Positioner>
    </Dialog.Root>
...
export function AssignmentDueDate({ assignment, showLateTokenButton = false }: { assignment: Assignment, showLateTokenButton?: boolean }) {
    ...
    return <HStack>
        ...
        {hoursExtended > 0 && <Text>({hoursExtended}-hour extension applied, {lateTokensConsumed} late tokens consumed)</Text>}
