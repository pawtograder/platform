...
import { Badge, Box, Card, Flex, Heading, HStack, Separator, Table, Text, VStack } from "@chakra-ui/react";
          ...
          <Card.Body>
            <VStack align="start" gap={2} py={4}>
              ...
              <Text color="fg.muted" fontSize="sm">
                An administrator binds the Canvas course to this class (after a launch captures it). Once linked, you
                can designate sections and enable roster and grade sync here.
              </Text>
            </VStack>
          </Card.Body>
        </Card.Root>
              ...
              {links.map((link, idx) => (
                <Box key={link.id}>
                  ...
                  <Text fontWeight="medium" mb={2}>
                    {link.context_title ?? link.context_label ?? link.context_id}
                  </Text>
                  <SectionMappingEditor
                    contextLinkId={link.id}
                    classId={courseId}
                    sectionRole={link.section_role}
                    classSectionId={link.class_section_id}
                    labSectionId={link.lab_section_id}
                    splitByMemberSection={link.split_by_member_section}
                    nrpsAvailable={!!link.nrps_url}
                    onChanged={load}
                  />
                </Box>
              ))}
