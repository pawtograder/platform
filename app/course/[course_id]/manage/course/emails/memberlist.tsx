...
import { Accordion, Box, Collapsible, Field, Flex, Text } from "@chakra-ui/react";
              ...
              <Accordion.ItemBody>
                {members && 
                    members.data.map((member, key) => {                 
                        return <Text key={key} fontSize="sm">{profiles.find((profile) => profile.id == member.profile_id)?.name}</Text>;
                        })}
                </Accordion.ItemBody>
            </Accordion.ItemContent>
          </Accordion.Item>
