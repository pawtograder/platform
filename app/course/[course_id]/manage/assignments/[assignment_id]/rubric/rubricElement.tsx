...
function hydratedRubricChecksToYamlRubric(checks: HydratedRubricCheck[]): YmlRubricChecksType[] {
  return checks
    ...
    .map((check) => ({
      ...
      points: check.points,
      comment_regex: valOrUndefined(check.comment_regex),
...
export type AssignmentWithRubric = Assignment & {
...
export default function RubricElement({ type }: { type: RubricType }) {
  ...
  const saveRubric = useCallback(
    async (value: string) => {
      console.log(assignment.data?.data.rubrics);
      console.log(existingRubric);
      ...
      await Promise.all(
        partChanges.toUpdate.map((part) =>
          updatePartIfChanged(
            part,
            existingRubric.rubric_parts.find((p: { id: number }) => p.id === part.id) as HydratedRubricPart
          )
        )
      );
        ...
        criteriaChanges.toUpdate.map((criteria) =>
          updateCriteriaIfChanged(
            ...
            existingRubric.rubric_parts
              .find((p: { id: number }) => p.id === criteria.rubric_part_id)
              ?.rubric_criteria.find((c: { id: number }) => c.id === criteria.id) as HydratedRubricCriteria
          )
        )
      );
      await Promise.all(
      ...
      await Promise.all(
        checkChanges.toUpdate.map((check) =>
          updateCheckIfChanged(
            check,
            allExistingChecks.find((c: { id: number }) => c.id === check.id) as HydratedRubricCheck
          )
        )
      );
      await Promise.all(
