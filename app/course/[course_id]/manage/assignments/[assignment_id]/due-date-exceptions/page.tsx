...
import { Assignment, AssignmentDueDateException, AssignmentGroup, AssignmentGroupMembersWithGroup, Course, UserProfile } from "@/utils/supabase/DatabaseTypes";
import { Box, Button, Dialog, Fieldset, Heading, HStack, Icon, Input, Skeleton, Table, Text, Textarea } from "@chakra-ui/react";
...
function AdjustDueDateDialog({ student, group, assignment, course, extensions }: { student: UserProfile, group?: AssignmentGroup, assignment: Assignment, course: Course, extensions: AssignmentDueDateException[] }) {
    ...
    const late_due_date = addHours(new TZDate(assignment.due_date!, course.time_zone || "America/New_York"), hoursExtended);
    ...
    return <Dialog.Root size="xl">
        ...
        <Dialog.Positioner>
            <Dialog.Content p={4}>
                ...
                <Dialog.Description>
                    The current due date for this {studentOrGroup} is {late_due_date.toLocaleString()} (an extension of {formattedDuration}).
                    You can manually adjust the due date for this {studentOrGroup} below, in increments of hours.
                    You can also choose to consume a late token when granting an extension, or could even grant extra tokens (with a negative number for tokens consumed).
                </Dialog.Description>
                <Dialog.Body>
                        ...
                        <Fieldset.Root bg="surface">
                            <Fieldset.Content w="xl">
                                ...
                                <Field label="Tokens to Consume"
                                    ...
                                    defaultValue={0}
                                >
                                    <Input type="number" {...register("tokens_consumed", { valueAsNumber: true, min: 0, required: "Tokens consumed is required" })} />
                                </Field>
                                <Field label="Notes"
                                ...
                                <Button type="submit" loading={isSubmitting} colorPalette="green">Add Due Date Exception</Button>
                            </Fieldset.Content>
                        </Fieldset.Root>
                    </form>
                    <Heading size="md">Extension History</Heading>
                    {extensions.length > 0 ? <Box maxH="400px" overflowY="auto">
                        <Table.Root maxW="2xl">
                            <Table.Header>
                                <Table.Row>
                            ...
                            <Table.Body>
                                {extensions.map(extension => <Table.Row key={extension.id}>
                                    <Table.Cell>{new TZDate(extension.created_at, course.time_zone || "America/New_York").toLocaleString()}</Table.Cell>
                                    <Table.Cell>{extension.hours}{
                                        <PopConfirm
                                            triggerLabel="Delete"
                                            trigger={<Button size="xs" variant="ghost" colorPalette="red"><Icon as={FaTrash} /></Button>}
                                            confirmText="Are you sure you want to delete this extension?"
                                            onConfirm={() => deleteException({ id: extension.id, resource: "assignment_due_date_exceptions" })}
                                            onCancel={() => { }}
                                            confirmHeader="Delete extension"
                                        />
                                    }</Table.Cell>
                                    <Table.Cell>{extension.tokens_consumed}</Table.Cell>
                                    <Table.Cell><PersonName showAvatar={false} uid={extension.creator_id} /></Table.Cell>
                                    <Table.Cell>{extension.note}</Table.Cell>
                                </Table.Row>)}
                            </Table.Body>
                        </Table.Root>
                    </Box> : <Text>No extensions have been granted for this {studentOrGroup}.</Text>}
                </Dialog.Body>
                <Dialog.Footer>
        ...
        <Table.Root>
            ...
            <Table.Body>
                {students.map(student => {
                    ...
                    const exceptions = dueDateExceptions?.data.filter(exception => (exception.student_id === student.id)
                    ...
                    const late_due_date = addHours(new TZDate(assignment.data.due_date!, course.classes.time_zone || "America/New_York"), hoursExtended);
                    ...
                    return <Table.Row key={student.id}>
                        ...
                        <Table.Cell><HStack>{hoursExtended > 0 ? hoursExtended : ""}{uniqueGrantors.map(grantor => <PersonAvatar key={grantor} uid={grantor} size="2xs" />)}</HStack></Table.Cell>
                        <Table.Cell>
                            <AdjustDueDateDialog student={student} group={group?.assignment_groups} assignment={assignment.data} course={course.classes} extensions={exceptions} />
                        </Table.Cell>
                    </Table.Row>
                })}
            </Table.Body>
