...
function AssignmentGroupsTable({ assignment, course_id }: { assignment: Assignment, course_id: number }) {
    const { data: groups, isLoading: groupsLoading, error: groupsError } = useList<AssignmentGroupWithMembersInvitationsAndJoinRequests>({
    ...
    const { data: profiles, isLoading: profilesLoading, error: profilesError } = useList<RolesWithProfilesAndGroupMemberships>({
        resource: "user_roles",
        meta: {
    ...
    const updateGroupForStudent = useCallback(async (group: AssignmentGroupWithMembersInvitationsAndJoinRequests | undefined, student: RolesWithProfilesAndGroupMemberships) => {
        try {
            setLoading(true);
    ...
    return <Box>
        ...
        <Text fontSize="sm" color="text.muted">This table allows you to tweak group configurations. <Text as="span" fontWeight="bold">Changes are applied in real time: use with caution.</Text></Text>
            ...
            <Table.Body>
                {profiles?.data?.map((profile) => {
                    ...
                    if (assignment.group_config === 'groups' && !group) {
                        ...
                        error = true;
                    } else if (group && assignment.min_group_size !== null && group.assignment_groups_members.length < assignment.min_group_size) {
                        errorMessage = `Group is too small (min: ${assignment.min_group_size}, current: ${group.assignment_groups_members.length})`;
                        error = true;
                    } else if (group && assignment.max_group_size !== null && group.assignment_groups_members.length > assignment.max_group_size) {
                    ...
                    return <Table.Row key={profile.id}>
                        ...
                        <Table.Cell>
                            <NativeSelect.Root disabled={loading}>
                                <NativeSelect.Field value={group?.id}
                                    onChange={(e) => {
                                    ...
                                    {groupsData?.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}
                                </NativeSelect.Field>
                            </NativeSelect.Root>
                        </Table.Cell>
