...
export default async function ManageAssignmentsPage({ params }: { params: Promise<{ course_id: string }> }) {
    ...
    const assignments = await client.from("assignments").select("*, submissions(profile_id, assignment_group_id)").eq("class_id", Number(course_id));

    let actions = <></>;
    actions = <HStack p={2}
    >
    <Button size="xs" asChild variant="solid" colorPalette="green"><NextLink href={`/course/${course_id}/manage/assignments/new`}>New Assignment</NextLink></Button>
