    ...
    <Flex height="100vh" overflow="hidden">
      <Stack spaceY="4" width="320px" borderEndWidth="1px" pt="6">
        ...
        <Stack mt="2" spaceY="4" flex="1" overflowY="auto" px="5" pb="5">
          <Stack mt="2" spaceY="4">
            <ChatGroupHeader icon={BsClipboardCheckFill}>
              Working ({requests?.filter((r) => r.assignee === private_profile_id).length})
            </ChatGroupHeader>
            <Stack spaceY="0" mx="-4">
              {requests
                ?.filter((r) => r.assignee === private_profile_id && r.resolved_by === null)
                .map((request) => (
          ...
          <Stack mt="2" spaceY="4">
            <ChatGroupHeader icon={BsChatTextFill}>
              Unassigned ({requests?.filter((r) => r.assignee === null).length})
            </ChatGroupHeader>
            <Stack spaceY="0" mx="-4">
              {requests
                ?.filter((r) => r.assignee === null && r.resolved_by === null)
                .map((request) => (
                  <NextLink href={`/course/${course_id}/manage/help/request/${request.id}`} key={request.id}>
                    <HelpRequestTeaser
                      data={{
          ...
          <Stack mt="2" spaceY="4">
            <ChatGroupHeader icon={BsChatTextFill}>
              Resolved ({requests?.filter((r) => r.resolved_by !== null).length})
            </ChatGroupHeader>
            <Stack spaceY="0" mx="-4">
              {requests
                ?.filter((r) => r.resolved_by !== null)
                .map((request) => (
                  <NextLink href={`/course/${course_id}/manage/help/request/${request.id}`} key={request.id}>
                    <HelpRequestTeaser
                      data={{
