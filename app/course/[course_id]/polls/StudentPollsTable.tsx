...
import { Box, Table, Text, Badge, HStack, Button } from "@chakra-ui/react";
...
export default function StudentPollsTable({ polls, onPollClick }: StudentPollsTableProps) {
    ...
    const getStatusBadge = (isLive: boolean) => {
        ...
        return (
            <Badge px={3} py={3} borderRadius="md" fontSize="md" fontWeight="medium" bg={colors.bg} color={colors.text}>
                {colors.label}
            </Badge>
        );
    };
        ...
        <Box border="1px solid" borderColor={tableBorderColor} borderRadius="lg" overflow="hidden">
            <Table.Root size="sm">
                <Table.Header bg={tableHeaderBg}>
                    <Table.Row>
                        <Table.ColumnHeader color={tableHeaderTextColor} fontWeight="semibold" textAlign="left" pl={4} pr={4} py={3}>
                            Question
                        </Table.ColumnHeader>
                        <Table.ColumnHeader color={tableHeaderTextColor} fontWeight="semibold" textAlign="right" pl={2} pr={4} py={3}>
                            Action
                        </Table.ColumnHeader>
                    </Table.Row>
                </Table.Header>
