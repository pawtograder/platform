...
export default function OfficeHoursSearchPage() {
  ...
  useEffect(() => {
    ...
    router.replace(`/course/${course_id}/office-hours/search${qs ? `?${qs}` : ""}`, { scroll: false });
  ...
  const availableQueues = helpQueues.filter((q) => q.available);

  return (
    <Box>
      <Flex align="center" gap="4" mb="6">
        <Button asChild variant="ghost" size="sm">
          <NextLink href={`/course/${course_id}/office-hours`}>
            <BsArrowLeft />
            Back
          </NextLink>
        </Button>
        ...
        <Box maxW="600px">
          <InputGroup startElement={<BsSearch />} w="full">
            <Input
              ...
              placeholder="Search by student name, request content, or queue..."
              size="lg"
              bg="bg"
            />
          </InputGroup>
