...
function TriggerWorkflowButton({ repository, sha }: { repository: string, sha: string }) {
    ...
    return <Button
        loading={isLoading}
    variant="outline" size="xs" onClick={async () => {
        setIsLoading(true);
        try {
...
function CommitHistory({ repository_id, repository_full_name }: { repository_id: number, repository_full_name: string }) {
    ...
    return <Box w="100%" bg="bg.muted" p={2} borderRadius="md" border="1px solid" borderColor="border.emphasized">
        ...
        <Table.Root>
            ...
            <Table.Body>
                {commits?.data.map((commit) => {
                    ...
                    return <Table.Row key={commit.sha}>
                        <Table.Cell>
                        ...
                        <Table.Cell>
                            {formatRelative(commit.status.commit_date || new Date(), new Date())}
                        </Table.Cell>
                        <Table.Cell>
                            {commit.status.commit_author}
                        </Table.Cell>
                        <Table.Cell>
                            {commit.commit_message}
                        </Table.Cell>
                        <Table.Cell>
                            {relatedSubmission ? <Link href={`/course/${relatedSubmission.class_id}/assignments/${relatedSubmission.assignments.id}/submissions/${relatedSubmission.id}`}>
                                {relatedSubmission.is_active && <ActiveSubmissionIcon />}
                                #{relatedSubmission.ordinal}, {relatedSubmission.grader_results?.score}/{relatedSubmission.assignments.autograder_points}
                            </Link> : <Box>
                                <Text fontSize="sm" color="text.muted">Not submitted</Text>
...
export function CommitHistoryDialog({ assignment, assignment_group_id, profile_id }: { assignment: Assignment, assignment_group_id: number | undefined, profile_id: string | undefined }) {
