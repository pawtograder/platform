...
import { assignmentGroupApproveRequest, assignmentGroupCreate, assignmentGroupJoin, assignmentGroupLeave, EdgeFunctionError } from "@/lib/edgeFunctions"
import { createClient } from "@/utils/supabase/client"
import { Assignment, AssignmentGroupInvitation, AssignmentGroupJoinRequest, AssignmentGroupMembersWithGroupMembersInvitationsAndJoinRequests, AssignmentGroupWithMembersInvitationsAndJoinRequests, UserProfile } from "@/utils/supabase/DatabaseTypes"
import { Avatar, Badge, Box, Button, Card, Dialog, Field, Flex, Heading, HStack, Icon, Input, List, ListItem, Separator, Skeleton, Tag, Text, VStack } from "@chakra-ui/react"
...
function CreateGroupButton({ assignment, allGroups }: { assignment: Assignment, allGroups: AssignmentGroupWithMembersInvitationsAndJoinRequests[] }) {
...
function useUngroupedProfiles(groups: AssignmentGroupWithMembersInvitationsAndJoinRequests[]) {
    ...
    const ungroupedProfiles = useMemo(() => {
        ...
        return profiles.filter(p =>
            ...
            !groups.some(g => g.assignment_groups_members.some(m => m.profile_id === p.id)));
    }, [profiles, groups]);
    return ungroupedProfiles;
}
function InviteButton({ group, allGroups }: { group: AssignmentGroupWithMembersInvitationsAndJoinRequests, allGroups: AssignmentGroupWithMembersInvitationsAndJoinRequests[] }) {
    ...
    return <Dialog.Root open={open} onOpenChange={e => {
        ...
        <Dialog.Trigger asChild>
            <Button variant="surface" colorPalette="green">Invite other students</Button>
        </Dialog.Trigger>
        <Dialog.Backdrop />
        <Dialog.Positioner>
            <Dialog.Content>
                <form>
                    ...
                    <Dialog.Body>
                        <Field.Root>
                            ...
                            {group.assignment_group_invitations.map(i => <AssignmentGroupInvitationView invitation={i} key={i.id} invalidateInvites={invalidateInvites} />)}
                        </Field.Root>
                        <Field.Root>
                            <Field.Label>Invite other students to join your group</Field.Label>
                            <Select
                        ...
                        You can optionally choose to invite other students to join your group.
                        Any student who has been enrolled in the course will also be able to request to join your group.
                    </Dialog.Body>
                    <Dialog.Footer>
                        <Dialog.ActionTrigger asChild>
                            <Button variant="ghost">Cancel</Button>
                        ...
                        <Button colorPalette="green" onClick={() => {
                            supabase.from('assignment_group_invitations').insert(selectedInvitees.map(i => ({
...
function GroupMemberList({ group }: { group: AssignmentGroupWithMembersInvitationsAndJoinRequests }) {
    const { profiles } = useClassProfiles();
    return <HStack>
        {group.assignment_groups_members.map(m => <Fragment key={m.profile_id}>
            <Text fontSize="sm" color="fg.muted">{profiles.find(p => p.id === m.profile_id)?.name}</Text>
        </Fragment>)}
    </HStack>
}
function JoinGroupButton({ groups, assignment }: { groups: AssignmentGroupWithMembersInvitationsAndJoinRequests[], assignment: Assignment }) {
    const { private_profile_id } = useClassProfiles();
    ...
    const myInvitations = useMemo(() => {
        const invitations = groups.map(g => g.assignment_group_invitations.map(i => ({
            ...i,
            group: g,
        }))).flat();
    ...
    const myRequests = useMemo(() => {
        ...
        )).flat().filter(j => j.profile_id === private_profile_id);
        return requests.sort((a, b) => {
            return b.created_at.localeCompare(a.created_at);
        });
    }, [groups, private_profile_id]);
    const myGroupsWithoutInvitationsOrRequests = useMemo(() => {
        return groups.filter(g => !g.assignment_group_invitations.some(i => i.invitee === private_profile_id) && !g.assignment_group_join_request.some(j => j.profile_id === private_profile_id && j.status === 'pending'));
    }, [groups, private_profile_id]);
    const invalidateInvites = useCallback(() => {
        invalidate({
    ...
    return <Dialog.Root open={open} onOpenChange={e => {
        ...
        <Dialog.Trigger asChild>
            <Button variant="surface" colorPalette="green">Join a Group</Button>
        </Dialog.Trigger>
        <Dialog.Backdrop />
        <Dialog.Positioner>
            <Dialog.Content>
                <form>
                    ...
                    <Dialog.Body>
                        <Field.Root>
                            ...
                            {myInvitations.length == 0 ? <Text fontSize="sm" color="fg.muted">You have no outstanding invitations.</Text> : <VStack >
                                {myInvitations.map(g => {
                                    return <VStack w="100%" key={g.id}>
                                        <HStack>
                                        ...
                                        <HStack w="100%" justifyContent="start">
                                            <Button colorPalette="green" onClick={async () => {
                                                try {
                                                    const { message } = await assignmentGroupJoin({ assignment_group_id: g.group.id }, supabase);
                                                    toaster.create({
                                                    ...
                                                    toaster.create({
                                                        title: "Error",
                                                        description: e instanceof EdgeFunctionError ? e.message : "An unknown error occurred",
                                                        type: "error",
                                                    });
                                                }
                                            }}>Accept</Button>
                                            <PopConfirm
                                                trigger={<Button variant="ghost" colorPalette="red">Decline</Button>}
                                                ...
                                                onConfirm={async () => {
                                                    try {
                                                        ...
                                                        const { error } = await supabase.from('assignment_group_invitations').delete().eq('id', myInvitation.id);
                                                        if (error) {
                                                            throw error;
                                                        }
                                                    }
                        ...
                        <Field.Root>
                            ...
                            <Select
                                ...
                                options={myGroupsWithoutInvitationsOrRequests
                                    .map(p => ({
                                        group: p,
                            ...
                            <Field.HelperText>
                                It is possible to request to join multiple groups at once, in which case you will be added to the first group that accepts your request.
                            </Field.HelperText>
                        </Field.Root>
                        {myRequests.length > 0 && <Field.Root>
                            <Field.Label>Prior Join Requests</Field.Label>
                            <VStack w="100%" maxH="50vh" overflowY="auto">
                                {myRequests.map(g => <Card.Root key={g.id} w="100%" bg={g.status === 'pending' ? 'bg.muted' : 'bg.subtle'}>
                                    <Flex flexDirection="row" p={2}>
                                        <VStack alignItems="flex-start">
                                            <Card.Title mb={0}>
                                                {g.group.name}
                                            </Card.Title>
                                            <Text fontSize="xs" color="fg.muted">({formatRelative(g.created_at, new Date())})</Text>
                                        </VStack>
                                        <VStack>
                                            <Text>{g.group.assignment_groups_members.length}/{assignment.max_group_size} members currently:</Text>
                                    ...
                                    <Card.Footer p={2}>
                                        <Flex justifyContent="space-between" alignItems="center" p={0} m={0} w="100%">
                                            <Flex alignItems="center" p={0} m={0}>
                                                ...
                                                <Text ml={2} fontSize="sm" color="fg.muted">{g.status.charAt(0).toUpperCase() + g.status.slice(1)}</Text>
                                            </Flex>
                                            {g.status === 'pending' && <Button colorPalette="green" onClick={async () => {
                                                const joinRequest = g;
                                                const { error } = await supabase.from('assignment_group_join_request').update(
                                                    {
                                                        status: 'withdrawn',
                                                ...
                                                toaster.create({
                                                    ...
                                                    description: "Your request to join this group has been cancelled. Note: the group may still have received a notification that you requested to join.",
                                                    type: "success",
                                                });
                                            }}>Withdraw</Button>}
                                        </Flex>
                            ...
                            <Field.HelperText>
                                A group member will need to approve your request to join their group, after which you will be added to the group.
                            </Field.HelperText>
                        </Field.Root>}
                    </Dialog.Body>
                    <Dialog.Footer>
...
function LeaveGroupButton({ assignment }: { assignment: Assignment }) {
    ...
    return <PopConfirm
        onCancel={() => { }}
        trigger={<Button variant="ghost" colorPalette="red">Leave group</Button>}
...
function GroupMember({ profile_id }: { profile_id: string }) {
    ...
    return <HStack>
        ...
        <Text fontSize="sm" color="fg.muted">{profile?.name}</Text>
    </HStack>
}
function InactiveJoinRequests({ group }: { group: AssignmentGroupWithMembersInvitationsAndJoinRequests }) {
    ...
    return <Dialog.Root>
        <Dialog.Trigger asChild><Button variant="ghost">View join request history</Button></Dialog.Trigger>
        <Dialog.Backdrop />
        <Dialog.Positioner>
            <Dialog.Content>
                <Dialog.Header>
                ...
                <Dialog.Body maxH="80vh" overflowY="auto">
                    {nonPendingRequests.map(j =>
                        <Card.Root key={j.id} p={2}>
                            <Card.Title><GroupMember profile_id={j.profile_id} /></Card.Title>
                            <Text fontSize="xs" color="fg.muted">({formatRelative(j.created_at, new Date())})</Text>
                            <Card.Body m={0} p={2}>
...
function AssignmentGroupJoinRequests({ group }: { group: AssignmentGroupWithMembersInvitationsAndJoinRequests }) {
    ...
    if (pendingRequests.length == 0) {
        return <Text fontSize="sm" color="fg.muted">There are no outstanding join requests from other students.</Text>
    }
    return <Box>
        <Heading size="sm">Approve or decline join requests</Heading>
        {group.assignment_group_join_request.filter(
            j => j.status === 'pending'
        ).map(j =>
...
function AssignmentGroupJoinRequestView({ join_request }: { join_request: AssignmentGroupJoinRequest }) {
    ...
    return <HStack>
        <Text fontSize="sm" color="fg.muted">{profile?.name} wants to join your group.</Text>
        <PopConfirm
            onConfirm={async () => {
                try {
            ...
            trigger={<Button size="xs" colorPalette="green">Approve</Button>}
        ...
        <PopConfirm
            ...
            trigger={<Button size="xs" colorPalette="red">Reject</Button>}
            triggerLabel="Reject"
...
function AssignmentGroupJoinRequestStatus({ join_request }: { join_request: AssignmentGroupJoinRequest }) {
    ...
    return <Box>
        <Text fontSize="sm" color="fg.muted">{join_request.status.charAt(0).toUpperCase() + join_request.status.slice(1)} by {decider?.name}{join_request.decided_at ? `, ${formatRelative(join_request.decided_at, new Date())}` : ''}</Text>
    </Box>
}
function AssignmentGroupInvitationView({ invitation, invalidateInvites }: { invitation: AssignmentGroupInvitation, invalidateInvites: () => void }) {
    const { private_profile_id } = useClassProfiles();
    const profile = useUserProfile(invitation.invitee);
    let actions = <></>
    if (invitation.inviter === private_profile_id) {
        actions = <Button variant="ghost" colorPalette="red" onClick={async () => {
            const supabase = createClient();
...
function GroupDetails({ group, allGroups, assignment }: { group: AssignmentGroupWithMembersInvitationsAndJoinRequests, allGroups: AssignmentGroupWithMembersInvitationsAndJoinRequests[], assignment: Assignment }) {
    return <VStack alignItems="flex-start">
        <Heading size="md">You are in group "{group.name}"</Heading>
        <HStack>
            {group.assignment_groups_members.map(m => <GroupMember profile_id={m.profile_id} key={m.profile_id} />)}
        </HStack>
...
export default function ManageGroupWidget({ assignment }: { assignment: Assignment }) {
    ...
    if (assignment.group_config === 'individual') {
        return <Box>
            ...
            <Text fontSize="sm" color="fg.muted">You will not be able to join a group for this assignment.</Text>
        </Box>
    }
