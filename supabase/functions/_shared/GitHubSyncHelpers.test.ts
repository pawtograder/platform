...
const { resetRefWithExpectedHead } = await import("./GitHubSyncHelpers.ts");
...
function fakeOctokit(onMutation: (call: GraphqlCall) => unknown) {
  ...
  return { octokit: octokit as unknown as Parameters<typeof resetRefWithExpectedHead>[0], calls };
}
...
Deno.test("a successful swap reports done, and tells GitHub the head it was authorized against", async () => {
  ...
  assertEquals(await resetRefWithExpectedHead(octokit, "org/repo-swap-ok", "sync-to-abc1234", EXPECTED, TARGET), true);
...
Deno.test("a rejected beforeOid reports not-done rather than throwing", async () => {
  ...
  assertEquals(
    await resetRefWithExpectedHead(octokit, "org/repo-swap-rejected", "sync-to-abc1234", EXPECTED, TARGET),
    false
  );
});
...
Deno.test("an unavailable mutation reports not-done, so the caller falls back", async () => {
  ...
  assertEquals(
    await resetRefWithExpectedHead(octokit, "org/repo-swap-unsupported", "sync-to-abc1234", EXPECTED, TARGET),
    false
  );
});
...
Deno.test("no repository id means no mutation is attempted at all", async () => {
  ...
  } as unknown as Parameters<typeof resetRefWithExpectedHead>[0];
  assertEquals(
    await resetRefWithExpectedHead(octokit, "org/repo-swap-no-id", "sync-to-abc1234", EXPECTED, TARGET),
    false
  );
...
Deno.test("the repository id is looked up once and reused", async () => {
  ...
  assertEquals(await resetRefWithExpectedHead(octokit, repo, "sync-to-abc1234", EXPECTED, TARGET), true);
  assertEquals(await resetRefWithExpectedHead(octokit, repo, "sync-to-def5678", EXPECTED, TARGET), true);
