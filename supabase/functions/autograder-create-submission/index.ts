...
async function handleRequest(req: Request) {
    ...
    if (repoData) {
        ...
        if (
            !workflow_ref.includes(`.github/workflows/grade.yml`,
            )
        ) {
            throw new Error(`Invalid workflow, got ${workflow_ref}`);
        }
        const { data: checkRun, error: checkRunError } = await adminSupabase.from("repository_check_runs")
            .select("*, user_roles(*), classes(time_zone)")
            .eq("repository_id", repoData.id).eq("sha", sha).maybeSingle();
        ...
        if (!checkRun.user_roles || (checkRun.user_roles.role !== "instructor" && checkRun.user_roles.role !== "grader")) {
            ...
            if (repoData.assignments.autograder?.max_submissions_period_secs && repoData.assignments.autograder?.max_submissions_count) {
                const { data: submissions, error: submissionsError } = await adminSupabase
                    ...
                    .eq("assignment_id", repoData.assignment_id)
                    .gte("created_at", addSeconds(new Date(), 0 - repoData.assignments.autograder.max_submissions_period_secs).toISOString())
                ...
                const submissionsInPeriod = submissions.filter(s =>
                    !s.grader_results ||
                    (s.grader_results && s.grader_results.score > 0)
                )
                console.log(`Rate limiter: ${submissionsInPeriod.length} / ${repoData.assignments.autograder.max_submissions_count} in ${repoData.assignments.autograder.max_submissions_period_secs} seconds`);
                if (submissionsInPeriod.length >= repoData.assignments.autograder.max_submissions_count) {
                    ...
                    const numSubmissionsOverLimit = 1 + submissionsInPeriod.length - repoData.assignments.autograder.max_submissions_count;
                    const oldestSubmission = submissionsInPeriod[submissionsInPeriod.length - numSubmissionsOverLimit];
                    const nextAllowedSubmission = addSeconds(new TZDate(oldestSubmission.created_at, timeZone), repoData.assignments.autograder.max_submissions_period_secs);
                    ...
                    throw new UserVisibleError(`Submission limit reached (max ${repoData.assignments.autograder.max_submissions_count} submissions per ${formatSeconds(repoData.assignments.autograder.max_submissions_period_secs)}). Please wait until ${format(nextAllowedSubmission, "MM/dd/yyyy HH:mm")} to submit again.`);
                }
            }
        }
