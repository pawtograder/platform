...
async function handleAssignmentGroupInstructorMoveStudent(req: Request): Promise<void> {
  const { new_assignment_group_id, old_assignment_group_id, profile_id, class_id } = await req.json() as AssignmentGroupInstructorMoveStudentRequest;
  ...
  if (old_assignment_group_id !== null) {
    ...
    const { data: currentGroup } = await supabase.from("assignment_groups_members")
