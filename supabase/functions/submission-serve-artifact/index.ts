...
const getArtifactZip = async (classId: string, profileId: string, submissionId: string, artifactId: string): Promise<Uint8Array> => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!targetFile && (filePath === '' || filePath.endsWith('/'))) {
        ...
        targetFile = zip.files.find((file: any) => 
          file.path === indexPath || file.path.endsWith(indexPath)
        );
      }
