-- Issue #545: optional note on score tweak (who/why).
-- Split-rubric (individual / assign-to-student) group grading: optional per-student tweak + note.

ALTER TABLE public.submission_reviews
  ADD COLUMN IF NOT EXISTS tweak_note text;

COMMENT ON COLUMN public.submission_reviews.tweak_note IS
  'Instructor note explaining the shared score tweak (visible to staff; optional).';

ALTER TABLE public.submission_reviews
  ADD COLUMN IF NOT EXISTS per_student_tweaks jsonb;

COMMENT ON COLUMN public.submission_reviews.per_student_tweaks IS
  'When rubric has individual/assign-to-student parts: optional map profile_id (uuid text) -> numeric tweak added to that student''s line only.';

ALTER TABLE public.submission_reviews
  ADD COLUMN IF NOT EXISTS per_student_tweak_notes jsonb;

COMMENT ON COLUMN public.submission_reviews.per_student_tweak_notes IS
  'When rubric has individual/assign-to-student parts: optional map profile_id (uuid text) -> note for that student''s tweak.';
...
COMMENT ON FUNCTION public._submission_review_recompute_scores(bigint) IS
  'Internal: recompute total_score, autograde, individual_scores, per_student_grading_totals, per_student_grading_shared_base; per_student_tweaks add to each student line and to total_score.';
...
AFTER UPDATE OF rubric_part_student_assignments, tweak, per_student_tweaks ON public.submission_reviews
FOR EACH ROW
...
COMMENT ON TRIGGER submission_reviews_recompute_split_metadata ON public.submission_reviews IS
  'Recompute scores when assign-to-student mapping, shared tweak, or per-student tweaks change; skipped for score-only updates from _submission_review_recompute_scores.';
