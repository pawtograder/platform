-- Gate the hand-grading score in `public.get_assignments_for_student_dashboard`
-- on `submission_reviews.released`.
--
-- Bug: the dashboard RPC is SECURITY DEFINER, so RLS does not fire inside it. The
-- `grading_review_for_submission` CTE joined `submission_reviews` unconditionally and
-- returned `grading_total_score` / `grading_submission_review_completed_at` regardless
-- of release status. The student dashboard then displayed the hand-grade score as soon
-- as grading was *completed*, before it was *released*.
--
-- The view this RPC replaced (security_invoker `assignments_for_student_dashboard`) was
-- protected by RLS: a student could not read an unreleased `submission_reviews` row, so
-- the join produced no score. The RPC lost that protection, leaking unreleased scores
-- both to real students and to instructors using view-as-student.
--
-- Fix: only surface the grading review when `sr.released = true`. When the review is
-- completed but not yet released, the CTE yields NULLs and the frontend falls back to
-- the autograder score — exactly the prior RLS-gated behavior. Authorization gate and
-- every other CTE are unchanged from 20260522130000_assignments_dashboard_rpc.sql.
