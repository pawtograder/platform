...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Could not create pgmq queue "notification_emails": %', SQLERRM;
END $$;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Could not grant table permissions or enable RLS: %', SQLERRM;
END $$;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'Could not grant function permissions: %', SQLERRM;
END $$;
