-- Fix bulk_assign_reviews to use explicit SELECT + INSERT/UPDATE
--
-- Previous attempts to use ON CONFLICT weren't working correctly. This fix uses
-- explicit SELECT to check for existing records, then either UPDATE or INSERT.
-- This ensures the same person being assigned multiple rubric parts for the same
-- submission will reuse the existing review_assignment row.
...
BEGIN
    ...
    LOOP
        ...
        -- FIX: Use explicit SELECT + INSERT/UPDATE instead of ON CONFLICT
        -- This avoids issues with constraint matching and ensures proper handling
        -- when the same person is assigned multiple rubric parts for the same submission_review
        SELECT id INTO v_review_assignment.id
        ...
        IF v_review_assignment.id IS NOT NULL THEN
        ...
        ELSE
            ...
            RETURNING id INTO v_review_assignment.id;
