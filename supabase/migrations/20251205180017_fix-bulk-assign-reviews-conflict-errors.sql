-- Fix ON CONFLICT errors in bulk_assign_reviews RPC
-- 
-- Issues fixed:
-- 1. ON CONFLICT clause was missing assignment_id and rubric_id, causing
--    "ON CONFLICT DO UPDATE command cannot affect row a second time" errors
--    when multiple entries had the same assignee/submission_review but different
--    assignment/rubric combinations
-- 2. Retargeting logic could insert duplicate (submission_id, rubric_id) pairs
--    when multiple review assignments retargeted to the same submission/rubric,
--    causing the same conflict error
...
BEGIN
    ...
    LOOP
        ...
        -- FIX: Use full unique constraint (assignee_profile_id, submission_review_id, assignment_id, rubric_id)
        -- instead of just (assignee_profile_id, submission_review_id) to prevent conflicts when
        -- multiple entries have the same assignee/submission_review but different assignment/rubric
    ...
    WITH candidate_ra AS (
        ...
        -- FIX: Use DISTINCT ON to prevent duplicate (submission_id, rubric_id) inserts
...
Fixed to use correct ON CONFLICT clause matching the unique constraint and prevent duplicate inserts in retargeting logic.';
