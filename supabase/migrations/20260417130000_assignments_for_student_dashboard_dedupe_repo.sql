-- assignments_for_student_dashboard joined repositories via a UNION of individual + group
-- repos, producing duplicate rows when a student had both. Pick a single repo per student
-- per assignment: match the chosen (latest) submission's repo when present; otherwise
-- prefer the group repo if any, then the individual repo.
...
WITH ur_students AS (
    ...
    SELECT cs.assignment_id,
           ...
           sr.total_score AS grading_total_score
    FROM chosen_submission cs
