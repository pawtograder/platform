...
-- 2a) Add readiness flag to repositories for GitHub provisioning lifecycle
do $$ begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'repositories' and column_name = 'is_github_ready'
  ) then
    alter table public.repositories add column is_github_ready boolean not null default false;
  end if;
end $$;
