-- Instructor "view as student" support for the student dashboard.
--
-- The student dashboard view (`public.assignments_for_student_dashboard`) is
-- `security_invoker=true`, and its `ur_students` CTE originally sourced from
-- `public.user_privileges` and filtered `WHERE role='student' AND user_id = auth.uid()`.
-- The only SELECT policy on `public.user_privileges` is the inlinable
--   `(user_id = auth.uid())`,
-- so an instructor querying the view for a student's profile id finds zero
-- ur_students rows and gets an empty dashboard.
--
-- This migration retargets the CTE to read from `public.user_roles`. The
-- existing SELECT policy "Enable users to view their own data only" on
-- `public.user_roles` already lets an instructor of a class read that class's
-- student rows (its second OR branch checks for the caller's instructor/grader
-- privilege in the row's class via a `user_privileges` sub-SELECT that is
-- satisfied by the inlinable `user_id = auth.uid()` policy — no recursion).
--
-- IMPORTANT: `public.user_privileges` RLS must stay strictly inlinable. We
-- intentionally do NOT add any function-based or otherwise opaque policy to
-- that hot table. The earlier draft of this migration added a policy and a
-- SECURITY DEFINER helper; both are dropped here so re-runs and fresh
-- environments converge to the same clean state.
--
-- Joined base tables (submissions, submission_reviews, repositories, etc.)
-- already permit instructor reads in their class via existing RLS, so the
-- security_invoker semantics carry through.

-- Clean up the earlier, rejected approach if it was applied locally.
DROP POLICY IF EXISTS "Instructors can read class privileges" ON public.user_privileges;
DROP FUNCTION IF EXISTS public.is_class_instructor_for_view_as(bigint);

-- Recreate the view, changing only the `ur_students` CTE source/WHERE.
...
WITH ur_students AS (
    ...
    WHERE ur.role = 'student'::public.app_role
      ...
      AND (
        -- Own student row (unchanged fast path for a normal student; their
        -- user_roles RLS limits the scan to this row anyway).
        ur.user_id = auth.uid()
        -- Or the caller is an instructor of that student (read-only view-as).
        -- authorizeforinstructorofstudent is SECURITY DEFINER, so the function
        -- call sits inside the view CTE (not in user_privileges RLS) and does
        -- not re-enter user_privileges' policy stack.
        OR public.authorizeforinstructorofstudent(ur.user_id)
      )
