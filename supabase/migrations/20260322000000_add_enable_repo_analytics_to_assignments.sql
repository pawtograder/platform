-- Add feature flag to control repo analytics per assignment (default disabled)
ALTER TABLE public.assignments
ADD COLUMN enable_repo_analytics boolean NOT NULL DEFAULT false;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE id = p_assignment_id AND class_id = p_class_id AND enable_repo_analytics = true
    ) THEN
        RAISE EXCEPTION 'Repo analytics not enabled for assignment %', p_assignment_id;
    END IF;
