...
alter table "public"."review_assignments" add column "submission_review_id" bigint;
...
DECLARE
    ...
    review_id bigint;
BEGIN
    FOR rec IN SELECT id, submission_id, rubric_id, class_id FROM review_assignments LOOP
        -- Check if a submission_review exists for this submission_id and rubric_id
        SELECT id INTO review_id
        FROM submission_reviews
        WHERE submission_id = rec.submission_id
          AND rubric_id = rec.rubric_id
        LIMIT 1;

        -- If not, create it
        IF review_id IS NULL THEN
            INSERT INTO submission_reviews (submission_id, rubric_id, class_id, total_score, tweak, released, name)
            VALUES (rec.submission_id, rec.rubric_id, rec.class_id, 0, 0, false, 'Autogenerated Review')
            RETURNING id INTO review_id;
        END IF;

        -- Update the review_assignment with the submission_review_id
        UPDATE review_assignments
        SET submission_review_id = review_id
        WHERE id = rec.id;
    END LOOP;
END $$;

alter table "public"."review_assignments" alter column "submission_review_id" set not null;
...
drop policy "Only graders and instructors can update" on "public"."submission_artifact_comments";

drop policy "Only graders and instructors can update" on "public"."submission_comments";

drop policy "Update self only" on "public"."submission_file_comments";

drop policy "only instructors and graders can edit" on "public"."submission_reviews";
...
create policy "Graders instructors update all, students update only before com"
on "public"."submission_artifact_comments"
as permissive
for update
to public
using ((authorizeforclassgrader(class_id) OR (authorizeforprofile(author) AND authorize_for_submission_review_writable(submission_review_id))));
...
create policy "instructors and gradersa all, self for not-complete review assi"
on "public"."submission_reviews"
as permissive
for update
to public
using ((authorizeforclassgrader(class_id) OR authorize_for_submission_review_writable(id)));
...
for insert
to public
with check ((authorizeforprofile(author) AND (authorizeforclassgrader(class_id) OR ((submission_review_id IS NULL) AND authorize_for_submission(submission_id)) OR authorize_for_submission_review_writable(submission_review_id))));
