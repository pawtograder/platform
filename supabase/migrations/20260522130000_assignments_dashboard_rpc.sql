-- Replace `public.assignments_for_student_dashboard` (security_invoker view) with a
-- SECURITY DEFINER RPC, `public.get_assignments_for_student_dashboard(class_id, student_profile_id)`.
--
-- Rationale:
--   * The view-as-student feature needs an instructor (or grader) to read a student's
--     dashboard data in their class. The view's `security_invoker=true` semantics, plus
--     the only-inlinable own-rows RLS policy on `public.user_privileges`, made that
--     impossible without either (a) a non-inlinable predicate on user_privileges
--     (rejected, see prior commits) or (b) restructuring `user_roles` RLS to avoid
--     recursion (broader change).
--   * Centralizing authorization at the top of an RPC is simpler, faster, and easier
--     to audit. Inside the function (SECURITY DEFINER) RLS does not fire, so the body
--     is plain SQL that can be planned and indexed cleanly. `public.user_privileges`
--     RLS remains strictly the inlinable `user_id = auth.uid()` policy.
--   * Per repo convention (CLAUDE.md): "Prefer Postgres RPCs over server actions or
--     Edge Functions for data operations."
--
-- Authorization gate: the caller is allowed when they are EITHER (a) the requested
-- student themselves, OR (b) an instructor/grader of the requested class. Both checks
-- read `public.user_roles` directly. Anything else raises 42501 (insufficient_privilege),
-- which Supabase clients surface as a permission error.
--
-- Frontend callers replace
--   supabase.from("assignments_for_student_dashboard").select("*")...
-- with
--   supabase.rpc("get_assignments_for_student_dashboard", { p_class_id, p_student_profile_id }).

-- Defensive cleanup: drop any leftover experimental policies/helpers from earlier
-- iterations of this work so the end state is deterministic. These are no-ops on
-- fresh environments.
DROP POLICY IF EXISTS "Instructors can read class privileges" ON public.user_privileges;
DROP POLICY IF EXISTS "Staff can read class privileges" ON public.user_privileges;
