...
for insert
to authenticated
with check (can_access_help_request(( SELECT help_request_messages.help_request_id
   FROM help_request_messages
  WHERE (help_request_messages.id = help_request_message_read_receipts.message_id))));
...
for select
to authenticated
using (can_access_help_request(( SELECT help_request_messages.help_request_id
   FROM help_request_messages
  WHERE (help_request_messages.id = help_request_message_read_receipts.message_id))));
...
for insert
to authenticated
with check ((authorizeforclassgrader(class_id) OR (authorizeforprofile(profile_id) AND authorizeforclass(class_id)) OR (authorizeforclass(class_id) AND ((EXISTS ( SELECT 1
   FROM help_requests hr
  WHERE ((hr.id = help_request_students.help_request_id) AND (hr.created_at > (now() - '00:02:00'::interval)) AND (hr.class_id = help_request_students.class_id)))) OR (EXISTS ( SELECT 1
...
for select
to authenticated
using ((authorizeforclassgrader(class_id) OR ((NOT is_private) AND authorizeforclass(class_id)) OR (is_private AND (auth.uid() = assignee)) OR (is_private AND user_is_in_help_request(id))));
