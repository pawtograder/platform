-- Add is_non_submission to the grader roster view: true when the active
-- submission is a content-less "grade anyway" stub (submitted_via='manual',
-- repository/sha NULL) on an assignment that expected a submission. The
-- repo_mode <> 'no_submission' gate excludes no_submission assignments, where
-- every student is a manual stub by design.
--
-- Reproduced from 20260707130000 with an added assignments join (for repo_mode)
-- and the computed is_non_submission column.
...
CREATE VIEW public.submissions_with_grades_for_assignment_nice WITH (security_invoker = 'true') AS
 ...
 SELECT swe.user_role_id AS id, swe.class_id, swe.assignment_id,
    ...
    COALESCE(
      (s.submitted_via = 'manual' AND s.repository IS NULL AND s.sha IS NULL AND a_rm.repo_mode <> 'no_submission'),
      false
    ) AS is_non_submission
   FROM submissions_with_extensions swe
     ...
     LEFT JOIN public.assignments a_rm ON ((a_rm.id = swe.assignment_id))
